...
export const emailService = {
  ...
  async sendEmail({ to, subject, html, text }) {
    ...
    try {
      ...
      const info = await mailer.sendMail({
        from: `"JeevanDoot Healthcare" <${env.SMTP_EMAIL}>`,
        to,
        subject,
        text,
        html,
  ...
  async sendBookingConfirmation({ patientEmail, patientName = 'Patient', doctorName, date, startTime, mode, calendarLink }) {
    ...
    const html = `
      <div style="font-family: sans-serif; padding: 24px; line-height: 1.6; max-width: 600px; margin: auto; border: 1px solid #E0E2EC; border-radius: 16px; background-color: #FAFAFA;">
        ...
        <p style="font-size: 13px; color: #555;">Best regards,<br/><strong>JeevanDoot Healthcare Team</strong></p>
      </div>
    `;
  ...
  async sendDoctorNewAppointmentAlert({ doctorEmail, doctorName, patientName, date, startTime, chiefComplaint, urgency, calendarLink }) {
    ...
    const html = `
      <div style="font-family: sans-serif; padding: 24px; line-height: 1.6; max-width: 600px; margin: auto; border: 1px solid #E0E2EC; border-radius: 16px; background-color: #FAFAFA;">
        ...
        <hr style="border: 0; border-top: 1px solid #E0E2EC; margin: 20px 0;" />
        <p style="font-size: 13px; color: #555;">Best regards,<br/><strong>JeevanDoot Clinical System</strong></p>
      </div>
    `;
  ...
  async sendCancellationAlert({ recipientEmail, recipientName, doctorName, patientName, date, startTime, reason, isDoctor = false }) {
    ...
    const html = `
      <div style="font-family: sans-serif; padding: 24px; line-height: 1.6; max-width: 600px; margin: auto; border: 1px solid #E0E2EC; border-radius: 16px; background-color: #FAFAFA;">
        ...
        ${!isDoctor ? `<p style="font-size: 14px;">You can rebook anytime using the JeevanDoot patient portal.</p>` : ''}
        <hr style="border: 0; border-top: 1px solid #E0E2EC; margin: 20px 0;" />
        <p style="font-size: 13px; color: #555;">Best regards,<br/><strong>JeevanDoot Care Team</strong></p>
      </div>
    `;
  ...
  async sendPasswordResetOTP({ email, name = 'User', otpCode }) {
    const subject = `JeevanDoot Security: Password Reset Code ${otpCode}`;
    ...
    const html = `
      <div style="font-family: sans-serif; padding: 24px; line-height: 1.6; max-width: 600px; margin: auto; border: 1px solid #E0E2EC; border-radius: 16px; background-color: #FAFAFA;">
        ...
        <p>We received a request to reset your password for your JeevanDoot account.</p>
        ...
        <hr style="border: 0; border-top: 1px solid #E0E2EC; margin: 20px 0;" />
        <p style="font-size: 13px; color: #555;">Best regards,<br/><strong>JeevanDoot Security Team</strong></p>
      </div>
    `;
